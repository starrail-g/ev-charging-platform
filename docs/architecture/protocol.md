...
## Accepted map-service extension (current runtime)
...
Schema v0.4, the three handlers on both the deterministic
`EV_MAP_SERVER_MOCK=1` path and the server-side Tencent HTTP path, map error
codes, station import, cache/audit persistence, and an internal-only simulator
gateway boundary. Production deployment work remains: asynchronous workers,
cache-miss coalescing, retention cleanup, and private mTLS transport.
