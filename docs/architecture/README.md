...
| [database.md](database.md) | 数据库设计（B 负责，待更新） |
| [protocol.md](protocol.md) | Socket 协议设计（B 负责，待更新） |

## 管理端/大屏在系统中的位置

- **管理端**：PC 端管理界面，经 `AdminRepository` 抽象访问数据；第一阶段默认 Mock，9/7 闸门后接入 Socket。边界见 [admin-client.md](admin-client.md)。
