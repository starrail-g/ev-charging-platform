...
class ClientServiceTest : public QObject {
  Q_OBJECT
private slots:
void loginValidation(){ MockUserService s; QVERIFY(s.login("").error.contains("手机号")); QVERIFY(s.login("bad").error.contains("手机号格式")); QVERIFY(s.login("12345678901").ok); QVERIFY(s.login("13800000000").ok); QVERIFY(s.login("13800000000", "ignored-password").ok); } void sessionGeneration(){ SessionManager s; User first{QStringLiteral("U1"),QStringLiteral("13800000000"),QStringLiteral("用户"),0,{}}; QCOMPARE(s.generation(),quint64(0)); s.beginSession(first); QCOMPARE(s.generation(),quint64(1)); const auto authGeneration=s.generation(); first.displayName=QStringLiteral("更新"); s.updateUser(first); QCOMPARE(s.generation(),authGeneration); s.setAvatarPath(QStringLiteral("avatar.png")); QCOMPARE(s.generation(),authGeneration); s.clear(); QCOMPARE(s.generation(),authGeneration+1); }
