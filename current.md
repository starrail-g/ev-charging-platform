...
- ① 已提交并推送（分支 `feature/stage2-pipeline`，head `812d31f`；**PR #24 已开，待 A 复核**）：c1 `3445be4`（`analytics/` 数据链）、c2 `039bf05`（`scripts/stage2/` 流水线与发布校验）、c3 `f0b9aa1`（大屏分析模式）、c4 `541f03e`（契约文档）；合并 `bc6d61e`（合入 `origin/main` `35c50ab`；冲突仅 `dashboard/js/app.js`×3 + `dashboard/css/app.css`×1，逐块解决）。f 批 7 笔（同分支，已推送）：`9657474` 跨日预测 / `c29f3b1` 窗口统一与 DWS 同源快照 / `fd55c7d` Dashboard 代理+标签+站点目录 / `77822f1` 一键启动指纹与裸环境加固 / `71dee12` 文档计数 / `7bc6310` 生成器 DQ05×DQ11 崩溃修复（含 low 档回归用例）/ `812d31f` 自举脚本 pyspark 有界安装。
- ② 仅本地工作树（**未提交**，待用户审查后按批次提交推送）：**A 对 PR #24 的 9 项评审修复批**——P1 ml 隔离 NULL 判定（`quality_valid` 显式非空 + `NULL_ENERGY/NULL_TOTAL/NULL_STATUS` 原因码 + 守恒/互斥/原因完备失败即中止）、P1 稀疏回退除零（全零预测安全评分 + 四份产物原子落盘）、P2 analytics 模式 ML 区标为独立数据来源（`js/analysis-source.js` + 页头/摘要/meta 三处标注）、P2 空结果分享链接“重置”恢复默认窗口（响应先写 coverage；默认窗口 = 最新 ≤90 天，`meta.default_*`）、P2 质量接口 batch_id 越界拒绝（结构化 400 `invalid_batch_id`）、P2 坏快照结构校验（统一 503 `snapshot_corrupt`）、P2 发布覆盖窗口 `available_*`（窗口末端结算收入可查询）、P2 README 快照命令补 `--dws`、P3 计数与状态口径；另含 `run_pipeline.sh` 裸环境 pyspark 回退链。**VM 全量复跑已完成**（见下证据行）。另有 `docs/meetings/interface-gate-2026-09-07.md` 遗留修改（非本批）。
- ③ 待办：修复批 commit/push（**等用户审查**）→ A 复核 PR #24 → 合入 `main`；A：PyCharm/录屏证据与贡献度材料；可选 G3（时段曲线）。
