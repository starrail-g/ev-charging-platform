...
- **G3 本轮验证**：BitDev 独立副本 `/home/bit/g3-review-20260916/repo` 生成器 **19/19**、覆盖窗口 **4/4**；本机 API **24/24**、Dashboard Node **60/60**、Dashboard Python **6/6**。smoke/low 同 seed 五 CSV 与标签逐字节一致；三个 profile 合法底稿边界、不重叠、活动唯一及缺测窗口检查通过；Windows/BitDev 五 CSV 哈希和分布统计一致。证据位于仓库外 `../build/g3-review-20260916/`（`audit.json`、`vm-audit.json`、测试日志），未纳入 Git。BitDev 缺少 PySpark/Spark，本轮未执行 Spark 清洗/数仓/发布链，不沿用 PR #24 的旧证据作为本轮结果；分支 `feature/g3-time-of-day-curves`，待 PR #25 复审。
