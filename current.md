...
- **G3（2026-09-15）**：生成器加入确定性工作日/周末小时权重，工作日强化 7–9、11–13、17–20，周末降低早高峰并强化午间/晚间；加权槽位保持单桩区间有序不重叠。新增时段曲线与槽位单元测试，生成器回归 **8/8**，Dashboard Node **60/60**；运行产物保存在仓库外 `.codex-work/`，未纳入 Git。
- **G3 修正（2026-09-16）**：起点改为槽位内按工作日/周末权重采样，修复 low 档峰谷被均匀抖动抹平；末槽位以上界 `data_end_exclusive` 约束，消除窗口越界；`GENERATOR_VERSION` 升至 **1.1.0**，并新增 low 档实际分布与窗口边界回归断言。
