...
- **G3 工作台与 ML 接入（2026-09-16）**：数仓新增清洗后用户/订单聚合事实；API 按站点/日期计算复购、RFM、订单统计，设备取批次末快照，营收与能耗沿用 ADS。未发布指标明确显示缺失，修复 `NaN%`、营收日期/卡片口径及窄屏溢出；口径见 `docs/api/analytics.md` §7。独立 ML 已训练并接入（42 预测、3 推荐、1 预警；MAE 16.2844、RMSE 30.7452），明确标注独立来源，不称为 G3 预测。BitDev 新快照 `/home/bit/ev-g3-workbench-artifacts` 保留原 G3 输入批次，旧发布未覆盖；VM URL `http://127.0.0.1:61469/?source=analytics&map=topology`，`bash /home/bit/g3-review-20260916/start-analysis.sh` 同时启动快照与 ML 服务。API/工作台 **30/30**、Node **63/63**、Dashboard Python **6/6**、Spark 数仓 **2/2**、ML 服务/输出 **12/12**、发布验收 **30/30**；HDFS 明细对账全网 500 RFM 样本，站点 1 / 9 月 1–13 日为 117 样本、12 复购，均与 API 一致；桌面六页及 390px 筛选/截图通过。证据位于仓库外 `../build/g3-review-20260916/`，待 PR #25 复审。
