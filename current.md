...
- `T-C1.1` review fixes (2026-09-03): local evidence = `node --test dashboard/tests/*.test.mjs` 25 passed, `python dashboard/serve.py --check` passed, Qt offscreen `tst_ui` 19 / `tst_launchsmoke` 6 / `tst_loginflow` 7 passed, CDP layout/interaction/pixel assertions at 1366/1920/900, screenshots `qa-shot-1366/1920/900.png` (repo-external). Not yet done: user acceptance on the working tree, commit split by task boundary, Ubuntu VM qmake6 re-run, PR #6 description refresh after CI.
