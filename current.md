...
- **G3 工作台与 ML 接入（2026-09-16）**：同批次清洗事实支持站点/日期筛选、复购、RFM 和订单统计，设备取批次末快照，营收/能耗沿用 ADS；缺失值不补零。全网 RFM 样本 500；站点 1 / 9 月 1–13 日为 117 样本、12 复购。独立 ML 已接入（42 预测、3 推荐、1 预警；MAE 16.2844、RMSE 30.7452），不称为 G3 预测。上一轮 Spark 数仓 2/2、ML 服务/输出 12/12 通过；本轮不重训模型。口径见 `docs/api/analytics.md` §7。
- **G3 图表与地图补全（2026-09-16）**：订单漏斗/设备状态中文化；补齐实际日期营收 OLS、固定利用率分组与同站点轴 Pareto、真实时长四桶、完成率控制限。设备累计充电次数总体 z-score 随站点筛选，67 台中 4 台统计偏离异常（不等于设备故障）；无方差时明确不可计算。腾讯 GL 从运行时 `TENCENT_MAP_JS_KEY` 读取配置，自动取景、响应容器尺寸变化并清理旧实例；无 key/离线/加载失败仍降级，真实 key 在仓库外。API/工作台 **34/34**、Node **66/66**、新 Spark ADS 重建/导出及发布 **30/30** 通过；DWD 独立对账时长桶全网 `[263,840,1603,3251]`（5957 单）、站点 1 / 9 月 1–13 日 `[5,15,39,69]`（128 单）。桌面六页、390px 筛选、腾讯底图及 VM Firefox 截图核验；评价事实/设备重启次数仍缺失。当前发布 `/home/bit/ev-g3-charts-artifacts`，原发布保留；VM URL `http://127.0.0.1:61469/?source=analytics`，启动 `bash /home/bit/g3-review-20260916/start-analysis.sh`。证据在仓库外 `../build/g3-review-20260916/`；本轮更新 PR #25，待复审。
...
- 2026-09-16：概览页布局稳定性修复（PR #26，`fix/dashboard-overview-layout`）：地图板块固定高度 `--ev-map-h: clamp(440px, 52vh, 620px)` 不再被同行右栏内容（974/1264px）拉伸；右栏三卡（实时关注/站点利用率/智能预测与调度）等分高度、各自内部滚动；全景态四张指标卡填满地图同高；左导航与内容列等长。仅 `dashboard/css/app.css` +28 行，规则限定 ≥901px（移动端 ≤900px 与工作台六页不受影响）；1920×1080 实测地图块 945×562、右栏三卡各 177px 滚动生效、指标卡各 128px 与地图齐平、左导航 1207px=内容列 1207px，页面总高 1752/2102→1340/1399；本地门禁 node 66/66、`serve.py --check` rc=0、servercfg+serve_proxy 6/6。
